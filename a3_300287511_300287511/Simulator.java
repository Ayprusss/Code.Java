...
public class Simulator {
	...
	public Simulator(ParkingLot lot, int perHourArrivalRate, int steps) {
		...
		if (lot.equals(null)) {
			throw new IllegalArgumentException("ParkingLot argument cannot be set to null. ");
		}
		else if (perHourArrivalRate <= 0) {
			throw new IllegalArgumentException("HourArrivalRate cannot be less or equal to zero. ");
		}
		else if (steps <= 0) {
			throw new IllegalArgumentException("simulation runTime(steps) cannot be a value less or equal to zero.");
		}
		...
		outgoingQueue = new LinkedQueue<Spot>();*/ // careful; not too sure if I should put this in the constructor or the simulate() method itself.
	...
	public void simulate() {
		...
		// Local variables can be defined here.
		 // end of the simulation would be done at 86400s.
		...
		while (clock < steps) {
			...
			if ( incomingQueue.peek() != null) {
				...
				Car inCar = incomingQueue.peek().getCar();

				if (lot.attemptParking(inCar, clock)) {
