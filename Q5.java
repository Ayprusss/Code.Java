public class Q5 {
    ...
    public static boolean isPrime(int x) {
        ...
        int i = 2;
        while(prime && x > i) {
            if(x % i == 0){ //meaning that the number is smaller than the divisor (i.e. 2/7)
                prime = false;
            }
            i++;
        }
    ...
    public static int getFibonacci(int position) {
        int num = 1; // at position 2
        int num2 = 2; // position 3
        int fibonacci = 3; //position 4
        ...
        return num; // num is the position
    }
}
